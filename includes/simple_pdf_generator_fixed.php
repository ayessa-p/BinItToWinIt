...
class SimplePDFGenerator {
    ...
    private $current_font = array('family' => 'Arial', 'size' => 12, 'style' => '');
    private $text_color = '0 0 0 rg';
    private $fill_color = '0.941 0.941 0.941 rg';
    ...
    public function Cell($w, $h, $txt, $border = 0, $ln = 0, $align = 'L') {
        ...
        $text_width = strlen($txt) * floatval($this->current_font['size']) * 0.6;
    ...
    public function Ln($h = null) {
        $this->y -= ($h !== null ? floatval($h) : floatval($this->current_font['size']) + 2);
    }
    
    public function MultiCell($w, $h, $txt) {
        $lines = explode("\n", wordwrap($txt, floor($w / (floatval($this->current_font['size']) * 6))));
    ...
    public function Header($txt, $font_size = 18, $center = true) {
        ...
        if ($center) {
            $text_width = strlen($txt) * floatval($font_size) * 0.6;
